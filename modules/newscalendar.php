...
class LmmCalendar extends Calendar{
  ...
  function getDateLink($day, $month, $year) {
    $conf = $this->conf;
    $link=null;
    $sql = "SELECT count(*) as `co` from $conf[DB_PREFIX]news where `DATE`>='$year-$month-$day' and `DATE` < ADDDATE( '$year-$month-$day' , 1 )";
    $q=mysql_query($sql);
    $r = mysql_fetch_assoc($q);
    if ($r[co]){
    //if(mktime (0,0,0,$month,$day,$year)<=time())
       $link = "$conf[rpath]/$day-$month-$year/";
    }
    return $link;
  }
};
...
class Calendar
{
  ...
  *********************************************************************************/
  ...
  /*
   ...
   represents Sunday.
   */
  var $dayNames = array("Вс", "Пн", "Вт", "Рс", "Чт", "Пт", "Сб");
...
class eeCalendar{
  ...
  function render($regionID = 0, $id, &$template) {
    $conf = $this->conf;
    $conf[rpath] = getFullPath($regionID);
    $cal = new LmmCalendar;
    $cal->setconf($conf);
  ...
  function printheader()
  {
    ...
    .'<center>'
    .'<center><a href="?a=viewnews&module=news">Просмотреть новости</a>&nbsp'
    .'<a href="?a=addnews&module=news">Добавить новость</a>&nbsp'
    .'<a href="?a=editouter&module=news" >Обрамление </a>&nbsp</center>'
    .'<a href="?a=type&module=news">Темы</a> '
    .'</center>'
...
$info = array(
  'plugin'      => "calendar",
  'cplugin'     => "eeCalendar",
  'pluginName'    => "Календарь для новостей",
  'ISMENU'      =>0,
  'ISENGINEMENU'    =>0,
  'ISBLOCK'     =>1,
  'ISEXTRABLOCK'    =>0,
